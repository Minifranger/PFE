...
public class KMean {
	...
	public static void main(String[] args) throws Exception {
		...
		//		}
		// execute program
		env.execute("Java KMean from KMean Example");
	}
	...
	public static final class MyWindowFunction implements WindowFunction<Tuple3<String, Integer, Integer>, Tuple4<String, Integer, Integer, Integer>, Tuple, GlobalWindow> {

		@Override
		public void apply(Tuple key, GlobalWindow window, Iterable<Tuple3<String, Integer, Integer>> input, Collector<Tuple4<String, Integer, Integer, Integer>> out) {

			int i = 1;
			ArrayList<Integer> distinctValue = new ArrayList<Integer>();

			//K=5
			for (Tuple3<String, Integer, Integer> points : input) {
				if (!distinctValue.contains(points.f2) || distinctValue.size() == 5) {
					distinctValue.add(points.f2);
					out.collect(Tuple4.of(points.f0, points.f1, points.f2, i));
					i++;
				} else {
					out.collect(Tuple4.of(points.f0, points.f1, points.f2, -1));
				}
			}
		}
	}
	 ...
	 * multiple pairs in the form of "(machine,1,1)" (Tuple3<String, Integer, Integer>).
	 */
	public static final class LineSplitter implements FlatMapFunction<String, Tuple3<String, Integer, Integer>> {
		...
		public void flatMap(String value, Collector<Tuple3<String, Integer, Integer>> out) {
			...
			if (tokens.length > 0) {
				out.collect(Tuple3.of(tokens[0], Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2])));
			}
